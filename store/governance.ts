...
import {
  TxResponse,
  Attribute,
} from "@kyvejs/types/client/cosmos/base/abci/v1beta1/abci";
...
export const useGovStore = defineStore("govtore", {
  ...
  getters: {
    ...
    getVotingProps(): Proposal[] {
      return Array.from(this.votingPeriodProps.values())
        .slice()
        .sort((a, b) => parseInt(a.id) - parseInt(b.id));
    },
    getEndedProps(): Proposal[] {
      return Array.from(this.endedProps.values())
        .slice()
        .sort((a, b) => parseInt(b.id) - parseInt(a.id)); //parseInt(b.id) - parseInt(a.id));
    },
    valoperVote: (state) => (id: string) => {
      console.log(
        "Validator vote for prop ",
        id,
        " is ",
        state.userVotes.get(id)
      );
    ...
    userVote: (state) => (id: string) => {
      ...
      if (state.userVotes.has(id)) {
        return state.userVotes.get(id);
      } else {
        return "VOTE_OPTION_NO_VOTE";
      }
    },
      ...
      (message): string => {
        ...
        try {
          ...
          switch (message_type) {
            ...
            case "/kyve.pool.v1beta1.MsgScheduleRuntimeUpgrade":
              return (
                `Upgrade pool runtime\n` +
                ` - runtime: ${message.runtime}\n` +
                ` - version: ${message.version}\n` +
                ` - time: ${moment
                  .unix(message.scheduled_at)
                  .format("MM/DD/YYYY")}`
              );
            default:
              return "";
  ...
  actions: {
    ...
    async fetchVotes(address: string) {
      ...
      const query = tx.GetTxsEventRequest.fromPartial({
        events: [
