...
export const useAppStore = defineStore("appStore", {
  ...
  getters: {
    ...
    staker_self_deleg(): string {
      ...
      return Number(
        Number(this.staker.self_delegation) / 10 ** this.sdk.config.coinDecimals
      ).toLocaleString();
    },
    ...
    staker_my_deleg(): string {
      ...
      return Number(
        Number(this.delegatorInfo.delegation_amount) /
          10 ** this.sdk.config.coinDecimals
      ).toLocaleString();
    },
    ...
    staker_my_rewards(): string {
      ...
      return Number(
        Number(this.delegatorInfo.current_reward) /
          10 ** this.sdk.config.coinDecimals
      ).toLocaleString();
    },
    ...
    consensus_my_rewards(): string {
      ...
      const reward = this.consensus_rewards.find(
        (type) => type.denom === this.sdk.config.coinDenom
      );
      return Number(
        Number(reward?.amount) / 10 ** this.sdk.config.coinDecimals
      ).toLocaleString();
    },
    dollar_consensus_rewards(): string {
      ...
      const price_value =
        this.price *
        Number(Number(reward?.amount) / 10 ** this.sdk.config.coinDecimals);
      return price_value.toLocaleString();
    },
    menu_items(): any {
      const items = [
  ...
  actions: {
    ...
    async init_store() {
      const index = cosmosConfig.findIndex(
    ...
    async delegate(amount: number, valkind, memo: string) {
      ...
      if (kind === "Protocol") {
        ...
        delegate = {
          ...
          value: CosmosDelegate.fromPartial({
            ...
            amount: {
              denom: this.sdk.config.coinDenom,
              amount: ukyveAmount.toString(),
            },
          }),
        };
      ...
      const result = await this.client.nativeClient.signAndBroadcast(
        this.walletAddress,
        [delegate],
        usedFee,
        memo
    ...
    async claim_rewards(
      ...
      if (commissions) {
        ...
        const staker_resp = await lcdClient.kyve.query.v1beta1.staker(
          QueryStakerRequest.fromPartial({ address: this.stakerAddress })
        );
        const ukyveCommissions =
          staker_resp.staker?.metadata?.commission_rewards;
      ...
      const usedFee = calculateFee(
        Math.round(gasEstimation * 1.4),
        GasPrice.fromString(
          this.sdk.config.gasPrice + this.sdk.config.coinDenom
        )
      );
      const result = await this.client.nativeClient.signAndBroadcast(
        this.walletAddress,
